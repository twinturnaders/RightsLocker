...
interface StateInfo {
  ...
  code: string;
  notes?: {
    recording?: string;   // e.g., one-party vs all-party consent
    statutes?: { label: string; cite: string; url?: string }[];
    cases?: { name: string; cite?: string; url?: string }[];
  };
}
@Component({
  standalone: true,
...
export class LegalComponent {
  ...
  q = '';
  states: { name: string; code: string; }[] = [{"name": "Alabama", "code": "AL"},
    {"name": "Alaska", "code": "AK"},
  ...
  current(): StateInfo | undefined {
    return this.states.find(s => s.code === this.q);
  }
}
