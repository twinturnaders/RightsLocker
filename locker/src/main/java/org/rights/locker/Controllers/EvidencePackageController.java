...
public class EvidencePackageController {
    ...
    public ResponseEntity<StreamingResponseBody> downloadPackage(@PathVariable UUID id,
                                                                 @RequestParam(defaultValue = "redacted") String type,
                                                                 @RequestParam(defaultValue = "true") boolean includeThumb) throws Exception {
        Evidence ev = evidenceRepo.findById(id).orElseThrow();

        // choose primary object
        final boolean hasRedacted = ev.getRedactedKey() != null;
        final String key = "original".equalsIgnoreCase(type) ? ev.getOriginalKey()
                : hasRedacted ? ev.getRedactedKey() : ev.getOriginalKey();
        final String bucket = "original".equalsIgnoreCase(type) ? bucketOriginals
        ...
        StreamingResponseBody body = outputStream -> {
            try (ZipOutputStream zip = new ZipOutputStream(outputStream)) {
                ...
                try (ResponseInputStream<GetObjectResponse> mediaIn =
                    ...
                    long mediaSize = copyToZip(zip, mediaIn, filenameBase + guessExt(key), mediaMd);
                    String mediaShaHex = HexFormat.of().formatHex(mediaMd.digest());

                    // 2) optional thumbnail
                    String thumbShaHex = null; long thumbSize = -1;
                    if (includeThumb && ev.getThumbnailKey() != null) {
                        try (ResponseInputStream<GetObjectResponse> thIn =
                            ...
                            thumbSize = copyToZip(zip, thIn, "thumb.jpg", thMd);
                            thumbShaHex = HexFormat.of().formatHex(thMd.digest());
                        } catch (Exception ignore) { /* thumbnail optional */ }
                    }
                    ...
                    // 6) optional HMAC attestation over metadata.json (simple)
    ...
    /* helpers: use java.io.InputStream instead of relying on .getBody() */
    ...
    private static long copyToZip(ZipOutputStream zip, ResponseInputStream<?> in, String name, MessageDigest md) throws IOException {
        zip.putNextEntry(new ZipEntry(name));
        byte[] buf = new byte[8192];
        int r; long size=0;
        while ((r = in.read(buf)) != -1) {
    ...
    private String buildHtmlReport(Evidence ev, String originalSha, long originalSize, boolean isRedacted, String deliveredSha, long deliveredSize) {
